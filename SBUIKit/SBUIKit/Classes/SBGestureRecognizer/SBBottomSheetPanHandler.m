//
//  SBBottomSheetPanHandler.m
//  SBUIKit
//
//  Created by 安康 on 2019/10/6.
//

#import "SBBottomSheetPanHandler.h"
...
@interface SBBottomSheetPanHandler ()
...
@implementation SBBottomSheetPanHandler
...
- (void)panAction:(UIPanGestureRecognizer *)gesture {
    ...
    afterY = self.handleView.sb_y + distance;
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.offsetChangeBlock) {
            self.offsetChangeBlock(afterY, offsetY, self.handleView.sb_h, offsetY < 0);
        }
        self.currentPoint = pointInSuperview;
...
- (void)autoBackLocation:(CGPoint)velocity {
    ...
    if (self.handleView.sb_y < self.minY * 0.5 + self.midY * 0.5) {
        if (velocity.y > 50) {
    ...
    }else if ((self.handleView.sb_y >= self.minY * 0.5 + self.midY * 0.5) &&
              (self.handleView.sb_y < self.maxY * 0.5 + self.midY * 0.5)) {
        if (velocity.y > 50) {
            self.directionUp = false;
            ...
            self.directionUp = self.handleView.sb_y > self.midY;
            self.levelType = BottomBarLevelTypeMid;
        }
    }else if (self.handleView.sb_y >= self.maxY * 0.5 + self.midY * 0.5) {
        if (velocity.y < -50) {
            self.directionUp = true;
            self.levelType = BottomBarLevelTypeMid;
        }else {
...
- (void)setLevelType:(BottomBarLevelType)levelType {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        if (levelType == BottomBarLevelTypeMin) {
            offsetY = self.handleView.sb_y - self.minY;
            self.handleView.sb_y = self.minY - self.extraHeight;
            if (self.offsetChangeBlock) {
                self.offsetChangeBlock(self.minY, offsetY, self.handleView.sb_h, self.directionUp);
            }
        }else if (levelType == BottomBarLevelTypeMid) {
            offsetY = self.handleView.sb_y - self.midY;
            self.handleView.sb_y = self.midY - self.extraHeight;
            if (self.offsetChangeBlock) {
                self.offsetChangeBlock(self.midY, offsetY, self.handleView.sb_h, self.directionUp);
            }
        }else if (levelType == BottomBarLevelTypeMax) {
            offsetY = self.handleView.sb_y - self.maxY;
            self.handleView.sb_y = self.maxY - self.extraHeight;
            if (self.offsetChangeBlock) {
                self.offsetChangeBlock(self.maxY, offsetY, self.handleView.sb_h, self.directionUp);
            }
        }
